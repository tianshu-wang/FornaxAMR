    ...
    int use_bf1)
{
    ...
    for (d = 0; d < 3; ++d) {
        bf_dst[d] = use_bf1 != 0 ? block->Bf1[d] : block->Bf[d];
    }
...
double prj_timeint_calc_dt(const prj_mesh *mesh, prj_eos *eos, const prj_mpi *mpi, double cfl)
{
    ...
    /* IMEX-SSP2(3,3,2) is stable at 2x the explicit CFL; the largest explicit
     * substep is dt/2, which then lands exactly at the per-cell CFL limit. */
    dt_global *= 2.0;
#endif
    return dt_global;
}
        ...
        if (prj_timeint_local_block(mpi, block)) {
            ...
            for (i = 0; i < PRJ_BLOCK_SIZE; ++i) {
                for (j = 0; j < PRJ_BLOCK_SIZE; ++j) {
                    for (k = 0; k < PRJ_BLOCK_SIZE; ++k) {
                        prj_timeint_update_cell_stage1_mhd_rad(mesh, rad, eos, block,
                            i, j, k, dt, grav, mpi, dt_src, block->W1, 1);
                    }
                }
            }
...
/* IMEX-SSP2(3,3,2) of Pareschi & Russo. The hyperbolic operator (fluxes,
 * geometric/gravity sources, radiation frequency advection and inelastic
 * scattering) is treated explicitly (F); the stiff radiation energy/momentum
 * coupling is treated implicitly (R). Each substep extracts its own operator
 * value relative to its entry state and accumulates it into the dUdt_ex /
 * dUdt_im / dBfdt_ex buffers; the three assembly steps reconstruct the stages
 * U^(1), U^(2), U^(3) and the final update U^(n+1) from U^n (held in W1/Bf1).
 *
 * With radiation disabled (PRJ_NRAD == 0) there is no implicit operator: the
 * implicit substep is skipped, dUdt_im stays zero, and the assembly reduces to
 * the explicit SSP(3,3) Runge-Kutta method. */

/* Conserved <-> primitive conversion that works for every build config. The
 * fused MHD+radiation helpers are inlined optimizations of prj_eos_prim2cons /
 * prj_eos_cons2prim (identical arithmetic), so dispatch to them when available
 * and fall back to the generic EOS routines otherwise. */
static inline void prj_timeint_imex_cons_from_prim(prj_eos *eos, const double *Wsrc,
    int i, int j, int k, double *u)
{
#if PRJ_MHD && PRJ_NRAD > 0
    (void)eos;
    prj_timeint_cell_cons_from_prim_mhd_rad(Wsrc, i, j, k, u);
#else
    double w[PRJ_NVAR_PRIM];

    prj_timeint_cell_prim(Wsrc, i, j, k, w);
    prj_eos_prim2cons(eos, w, u);
#endif
}

static inline void prj_timeint_imex_store(prj_eos *eos, double *Wdst,
    int i, int j, int k, double *u)
{
#if PRJ_MHD && PRJ_NRAD > 0
    (void)eos;
...
static void prj_timeint_imex_save_and_zero(prj_mesh *mesh, const prj_mpi *mpi)
{
    int bidx;
    ...
    for (bidx = 0; bidx < mesh->nblocks; ++bidx) {
        ...
        for (n = 0; n < prim_count; ++n) {
            block->W1[n] = block->W[n];
        }
        for (n = 0; n < cons_count; ++n) {
            block->dUdt_ex[n] = 0.0;
            block->dUdt_im[n] = 0.0;
        }
#if PRJ_MHD
        {
            ...
            for (d = 0; d < 3; ++d) {
                for (n = 0; n < (size_t)PRJ_BLOCK_NFACES; ++n) {
                    block->Bf1[d][n] = block->Bf[d][n];
                    block->dBfdt_ex[d][n] = 0.0;
                }
            }
        }
#endif
...
#if PRJ_NRAD > 0
static void prj_timeint_imex_im_cell(prj_rad *rad, prj_eos *eos, prj_block *block,
    int i, int j, int k, double dt_sub)
{
    double u[PRJ_NVAR_CONS];
    double u_entry[PRJ_NVAR_CONS];
    double T_cell = block->eosvar[EIDX(PRJ_EOSVAR_TEMPERATURE, i, j, k)];
    double lapse_cell = prj_timeint_cell_lapse(block, i, j, k);
    double kappa[PRJ_NRAD * PRJ_NEGROUP];
    ...
    prj_rad_momentum_update(rad, eos, u, dt_sub, lapse_cell, T_cell, kappa);
    prj_timeint_imex_store(eos, block->W, i, j, k, u);
    for (v = 0; v < PRJ_NVAR_CONS; ++v) {
        block->dUdt_im[VIDX(v, i, j, k)] += (u[v] - u_entry[v]) * inv_dt;
    }
}
#endif

static void prj_timeint_imex_ex_cell(const prj_mesh *mesh, prj_rad *rad, prj_eos *eos,
    prj_block *block, int i, int j, int k, double dt_sub, const prj_grav *grav,
    const prj_mpi *mpi, double *dt_src)
{
    double u[PRJ_NVAR_CONS];
    double u_entry[PRJ_NVAR_CONS];
    ...
    for (v = 0; v < PRJ_NVAR_CONS; ++v) {
        u_entry[v] = u[v];
        u[v] += dt_sub * (block->dUdt[VIDX(v, i, j, k)] + fluxdiv[v]);
    }
    ...
    prj_timeint_imex_store(eos, block->W, i, j, k, u);
    for (v = 0; v < PRJ_NVAR_CONS; ++v) {
        block->dUdt_ex[VIDX(v, i, j, k)] += (u[v] - u_entry[v]) * inv_dt;
    }
}

static void prj_timeint_imex_assemble_block(prj_eos *eos, prj_block *block,
    double w_ex, double w_im)
{
    int i;
    int j;
    int k;
...
#if PRJ_MHD
    /* Reconstruct the face magnetic field first so that the cell loop reads a
     * fully assembled Bf when it derives the cell-centered B. */
    {
        int dir;
            ...
            for (i = 0; i <= prj_timeint_mhd_face_axis_max(dir, 0); ++i) {
                for (j = 0; j <= prj_timeint_mhd_face_axis_max(dir, 1); ++j) {
                    for (k = 0; k <= prj_timeint_mhd_face_axis_max(dir, 2); ++k) {
                        ...
                        block->Bf[dir][idx] =
                            block->Bf1[dir][idx] + w_ex * block->dBfdt_ex[dir][idx];
                    }
                }
            }
        }
    ...
    for (i = 0; i < PRJ_BLOCK_SIZE; ++i) {
        for (j = 0; j < PRJ_BLOCK_SIZE; ++j) {
            for (k = 0; k < PRJ_BLOCK_SIZE; ++k) {
                ...
                prj_timeint_imex_cons_from_prim(eos, block->W1, i, j, k, u);
                for (v = 0; v < PRJ_NVAR_CONS; ++v) {
                    u[v] += w_ex * block->dUdt_ex[VIDX(v, i, j, k)] +
                            w_im * block->dUdt_im[VIDX(v, i, j, k)];
                }
#if PRJ_MHD
                /* B is carried by the CT face field, not dUdt_ex; overwrite the
                 * cell-centered components from the assembled Bf. */
                prj_timeint_mhd_set_cons_b_from_bf(block, block->Bf, i, j, k, u);
#endif
                prj_timeint_imex_store(eos, block->W, i, j, k, u);
            }
...
static void prj_timeint_imex_fill_after_W(prj_mesh *mesh, const prj_bc *bc, prj_eos *eos,
    prj_rad *rad, prj_grav *grav, prj_mpi *mpi)
{
    prj_eos_fill_active_cells(mesh, eos, mpi, 1, PRJ_EOS_CTX_MAIN);
    prj_boundary_fill_ghosts_and_bf(mesh, mpi, bc, 1, 0, eos, grav, rad,
        PRJ_BOUNDARY_TIMER_SCOPE_NONE);
...
#if PRJ_NRAD > 0
    /* The implicit operator is the radiation energy/momentum coupling. With no
     * radiation there is nothing to solve, so the cell update is skipped and W
     * is left unchanged (a pure explicit method). */
    {
        int bidx;

        PRJ_TIMER_BARRIER_START(timer, mpi, "imex_im_cell_update");
        for (bidx = 0; bidx < mesh->nblocks; ++bidx) {
            prj_block *block = &mesh->blocks[bidx];
            int i;
            int j;
            ...
            for (i = 0; i < PRJ_BLOCK_SIZE; ++i) {
                for (j = 0; j < PRJ_BLOCK_SIZE; ++j) {
                    for (k = 0; k < PRJ_BLOCK_SIZE; ++k) {
                        prj_timeint_imex_im_cell(rad, eos, block, i, j, k, dt_sub);
                    }
                }
            }
        }
        PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_im_cell_update");
    }
#else
    (void)dt_sub;
#endif

    /* Refresh ghosts/EOS/opacity/gravity for W (changed here or by the
     * preceding assembly) so the following step_ex sees a consistent state. */
    PRJ_TIMER_BARRIER_START(timer, mpi, "imex_im_fill");
    prj_timeint_imex_fill_after_W(mesh, bc, eos, rad, grav, mpi);
    PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_im_fill");
#else
    (void)mesh;
    (void)coord;
    (void)bc;
    ...
    prj_rad *rad, prj_grav *grav, prj_mpi *mpi, double dt_sub, double *dt_src, prj_timer *timer)
{
    ...
    (void)coord;
    (void)bc; /* step_ex does no ghost/eos fills; the following step_im does. */

    PRJ_TIMER_BARRIER_START(timer, mpi, "imex_ex_flux_send");
    for (bidx = 0; bidx < mesh->nblocks; ++bidx) {
        prj_block *block = &mesh->blocks[bidx];
    ...
    prj_riemann_flux_send(mesh, mpi);
    PRJ_SUBTIMER_STOP("sub_riemann_flux_send");
    PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_ex_flux_send");

#if PRJ_MHD
    PRJ_TIMER_BARRIER_START(timer, mpi, "imex_ex_mhd_emf");
    prj_timeint_mhd_update_mesh_emf(mesh, mpi, prj_timeint_stage1_array);
    PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_ex_mhd_emf");
#endif

    PRJ_TIMER_BARRIER_START(timer, mpi, "imex_ex_flux_emf_exchange");
    prj_mpi_exchange_fluxes_and_emf(mesh, mpi);
    PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_ex_flux_emf_exchange");

#if PRJ_MHD
    PRJ_TIMER_BARRIER_START(timer, mpi, "imex_ex_mhd_update_bf");
    for (bidx = 0; bidx < mesh->nblocks; ++bidx) {
        prj_block *block = &mesh->blocks[bidx];
        int dir;
        size_t n;
        ...
        /* Snapshot Bf at entry, advance it in place (stage 0: Bf -> Bf), then
         * extract the explicit B operator relative to the entry field. */
        for (dir = 0; dir < 3; ++dir) {
            for (n = 0; n < (size_t)PRJ_BLOCK_NFACES; ++n) {
                block->dBfdt_ex[dir][n] -= block->Bf[dir][n] / dt_sub;
            }
        }
        prj_timeint_mhd_update_bf(block, 0, dt_sub);
        for (dir = 0; dir < 3; ++dir) {
            for (n = 0; n < (size_t)PRJ_BLOCK_NFACES; ++n) {
                block->dBfdt_ex[dir][n] += block->Bf[dir][n] / dt_sub;
            }
    ...
    PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_ex_mhd_update_bf");
#endif

    PRJ_TIMER_BARRIER_START(timer, mpi, "imex_ex_src_cell_update");
    for (bidx = 0; bidx < mesh->nblocks; ++bidx) {
        prj_block *block = &mesh->blocks[bidx];
        int i;
        int j;
        ...
        prj_src_update(eos, rad, grav, block, block->W, block->dUdt);
        PRJ_SUBTIMER_STOP("sub_cell_src_update");
        PRJ_SUBTIMER_START("sub_cell_imex_ex_update");
        for (i = 0; i < PRJ_BLOCK_SIZE; ++i) {
            for (j = 0; j < PRJ_BLOCK_SIZE; ++j) {
                for (k = 0; k < PRJ_BLOCK_SIZE; ++k) {
                    prj_timeint_imex_ex_cell(mesh, rad, eos, block, i, j, k, dt_sub,
                        grav, mpi, dt_src);
                }
            }
        }
        PRJ_SUBTIMER_STOP("sub_cell_imex_ex_update");
    }
    PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_ex_src_cell_update");
#else
    (void)mesh;
    (void)coord;
    (void)bc;
...
#if TIME_INTEGRATION == PRJ_TIMEINT_IMEX_SSP2332
static void prj_timeint_imex_assemble(prj_mesh *mesh, prj_eos *eos, const prj_mpi *mpi,
    double w_ex, double w_im, prj_timer *timer, const char *timer_name)
{
    int bidx;

    PRJ_TIMER_BARRIER_START(timer, mpi, timer_name);
    for (bidx = 0; bidx < mesh->nblocks; ++bidx) {
        prj_block *block = &mesh->blocks[bidx];

        if (prj_timeint_local_block(mpi, block)) {
            prj_timeint_imex_assemble_block(eos, block, w_ex, w_im);
        }
    }
    PRJ_TIMER_BARRIER_STOP(timer, mpi, timer_name);
}

static void prj_timeint_imex_advance(prj_mesh *mesh, const prj_coord *coord, const prj_bc *bc,
    prj_eos *eos, prj_rad *rad, prj_grav *grav, prj_mpi *mpi, double dt, double *dt_src,
    prj_timer *timer)
{
    prj_timeint_imex_save_and_zero(mesh, mpi);

    /* Stage 1: U^(1) = U^n + (1/4) dt R(U^(1)). */
    prj_timeint_step_im(mesh, coord, bc, eos, rad, grav, mpi, 0.25 * dt, dt_src, timer);
    prj_timeint_step_ex(mesh, coord, bc, eos, rad, grav, mpi, 0.5 * dt, dt_src, timer);

    /* W <- U^n + (1/2) dt F(U^(1)) (explicit part of U^(2)). */
    prj_timeint_imex_assemble(mesh, eos, mpi, 0.5 * dt, 0.0, timer, "imex_assemble1");

    /* Stage 2: U^(2) = (explicit part) + (1/4) dt R(U^(2)). */
    prj_timeint_step_im(mesh, coord, bc, eos, rad, grav, mpi, 0.25 * dt, dt_src, timer);
    prj_timeint_step_ex(mesh, coord, bc, eos, rad, grav, mpi, 0.5 * dt, dt_src, timer);

    /* W <- U^n + (1/2) dt [F(1)+F(2)] + (1/3) dt [R(1)+R(2)] (explicit part of U^(3)). */
    prj_timeint_imex_assemble(mesh, eos, mpi, 0.5 * dt, (1.0 / 3.0) * dt, timer, "imex_assemble2");

    /* Stage 3: U^(3) = (explicit part) + (1/3) dt R(U^(3)). */
    prj_timeint_step_im(mesh, coord, bc, eos, rad, grav, mpi, (1.0 / 3.0) * dt, dt_src, timer);
    prj_timeint_step_ex(mesh, coord, bc, eos, rad, grav, mpi, (1.0 / 3.0) * dt, dt_src, timer);

    /* U^(n+1) = U^n + (1/3) dt [sum F] + (1/3) dt [sum R]. */
    prj_timeint_imex_assemble(mesh, eos, mpi, (1.0 / 3.0) * dt, (1.0 / 3.0) * dt, timer,
        "imex_assemble_final");

    PRJ_TIMER_BARRIER_START(timer, mpi, "imex_final_fill");
    prj_timeint_imex_fill_after_W(mesh, bc, eos, rad, grav, mpi);
    PRJ_TIMER_BARRIER_STOP(timer, mpi, "imex_final_fill");
}
#endif
