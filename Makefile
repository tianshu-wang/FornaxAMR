...
CPPFLAGS := -Isrc -DPRJ_ENABLE_MPI -DPRJ_USE_GRAVITY=$(GRAVITY) -DPRJ_GRAV_DEBUG=$(GRAV_DEBUG) -DPRJ_TIMER=$(TIMER) -DPRJ_USE_RADIATION=$(RADIATION) -DPRJ_MHD=$(MHD) -DPRJ_MHD_DEBUG=$(MHD_DEBUG) -DPRJ_LHLLD_RIEMANN=$(LHLLD_RIEMANN) -DPRJ_DUMP_SINGLE_PRECISION=$(DUMP_SINGLE_PRECISION) -DPRJ_MIXED_PRECISION=$(MIXED_PRECISION) $(RECON_HYDRO_DEF) $(RECON_RADIATION_DEF) $(NGHOST_DEF) $(NRAD_DEF) $(NEGROUP_DEF) $(GIT_DEFS) $(MPI_CFLAGS) $(HDF5_CFLAGS) $(MACHINE_CPPFLAGS)
