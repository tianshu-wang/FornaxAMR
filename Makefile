CC ?= $(shell command -v mpicc >/dev/null 2>&1 && echo mpicc || echo cc)
STD := -std=c99
WARN := -Wall -Wextra -pedantic

MACHINE_MK := machine.mk
SETUP_MK := setup.mk

GIT_BRANCH := $(shell git rev-parse --abbrev-ref HEAD 2>/dev/null || echo unknown)
GIT_COMMIT := $(shell git rev-parse HEAD 2>/dev/null || echo unknown)
GIT_DEFS := -DPRJ_BRANCH='"$(GIT_BRANCH)"' -DPRJ_COMMIT='"$(GIT_COMMIT)"'

ifneq ($(MAKECMDGOALS),clean)
ifeq ($(wildcard $(MACHINE_MK)),)
$(error $(MACHINE_MK) is required; link or copy one from machines/*.mk)
endif
ifeq ($(wildcard $(SETUP_MK)),)
$(error $(SETUP_MK) is required; link or copy one from setups/*.mk)
endif
include $(MACHINE_MK)
include $(SETUP_MK)
endif

GRAVITY ?= 1
GRAV_DEBUG ?= 0
TIMER ?= 0
RADIATION ?= 0
MHD ?= 0
MHD_DEBUG ?= 0
LHLLD_RIEMANN ?= 1
TIME_INTEGRATION ?= IMEX_SSP2332
DUMP_SINGLE_PRECISION ?= 1
MIXED_PRECISION ?= 0
RECON ?=
RECON_HYDRO ?=
RECON_RADIATION ?=
NGHOST ?=
NRAD ?=
NEGROUP ?=
MPI_CFLAGS ?= $(shell $(CC) --showme:compile 2>/dev/null)
MPI_LIBS ?= $(shell $(CC) --showme:link 2>/dev/null)
HDF5_CFLAGS ?= $(shell pkg-config --cflags hdf5)
HDF5_LIBS ?= $(shell pkg-config --libs hdf5)
MACHINE_CPPFLAGS ?=
MACHINE_CFLAGS ?=
MACHINE_LDFLAGS ?=
MACHINE_LDLIBS ?=
OMPFLAGS ?=
TIME_INTEGRATION_VALUE := $(strip $(TIME_INTEGRATION))
TIME_INTEGRATION_DEF := -DTIME_INTEGRATION=$(TIME_INTEGRATION_VALUE)
ifneq ($(MAKECMDGOALS),clean)
ifeq ($(TIME_INTEGRATION_VALUE),eSSPRK9_3)
TIME_INTEGRATION_DEF := -DTIME_INTEGRATION=$(TIME_INTEGRATION_VALUE) -D$(TIME_INTEGRATION_VALUE)=PRJ_TIMEINT_ESSPRK9_3
else
ifeq ($(TIME_INTEGRATION_VALUE),IMEX_SSP2332)
TIME_INTEGRATION_DEF := -DTIME_INTEGRATION=$(TIME_INTEGRATION_VALUE) -D$(TIME_INTEGRATION_VALUE)=PRJ_TIMEINT_IMEX_SSP2332
else
ifneq ($(TIME_INTEGRATION_VALUE),RK2)
ifneq ($(filter eSSPRK%_2,$(TIME_INTEGRATION_VALUE)),)
TIME_INTEGRATION_N := $(patsubst eSSPRK%_2,%,$(TIME_INTEGRATION_VALUE))
ifneq ($(shell expr '$(TIME_INTEGRATION_N)' : '[0-9][0-9]*$$' >/dev/null && [ '$(TIME_INTEGRATION_N)' -ge 2 ] && echo yes || echo no),yes)
$(error TIME_INTEGRATION=$(TIME_INTEGRATION_VALUE) requires N >= 2)
endif
TIME_INTEGRATION_DEF := -DTIME_INTEGRATION=$(TIME_INTEGRATION_VALUE) -D$(TIME_INTEGRATION_VALUE)=PRJ_TIMEINT_ESSPRK -DPRJ_TIMEINT_ESSPRK_N=$(TIME_INTEGRATION_N)
else
$(error Unsupported TIME_INTEGRATION=$(TIME_INTEGRATION_VALUE))
endif
endif
endif
endif
endif
NRAD_DEF := $(if $(NRAD),-DPRJ_NRAD=$(NRAD),)
NEGROUP_DEF := $(if $(NEGROUP),-DPRJ_NEGROUP=$(NEGROUP),)
RECON_HYDRO_DEF := $(if $(RECON_HYDRO),-DPRJ_RECON_HYDRO=$(RECON_HYDRO),$(if $(RECON),-DPRJ_RECON_HYDRO=$(RECON),))
RECON_RADIATION_DEF := $(if $(RECON_RADIATION),-DPRJ_RECON_RADIATION=$(RECON_RADIATION),)
NGHOST_DEF := $(if $(NGHOST),-DPRJ_NGHOST=$(NGHOST),)
CPPFLAGS := -Isrc -DPRJ_ENABLE_MPI -DPRJ_USE_GRAVITY=$(GRAVITY) -DPRJ_GRAV_DEBUG=$(GRAV_DEBUG) -DPRJ_TIMER=$(TIMER) -DPRJ_USE_RADIATION=$(RADIATION) -DPRJ_MHD=$(MHD) -DPRJ_MHD_DEBUG=$(MHD_DEBUG) -DPRJ_LHLLD_RIEMANN=$(LHLLD_RIEMANN) $(TIME_INTEGRATION_DEF) -DPRJ_DUMP_SINGLE_PRECISION=$(DUMP_SINGLE_PRECISION) -DPRJ_MIXED_PRECISION=$(MIXED_PRECISION) $(RECON_HYDRO_DEF) $(RECON_RADIATION_DEF) $(NGHOST_DEF) $(NRAD_DEF) $(NEGROUP_DEF) $(GIT_DEFS) $(MPI_CFLAGS) $(HDF5_CFLAGS) $(MACHINE_CPPFLAGS)
LDFLAGS := $(MACHINE_LDFLAGS)
LDLIBS := $(HDF5_LIBS) $(MPI_LIBS) $(MACHINE_LDLIBS)

ifeq ($(DEBUG),1)
CFLAGS := $(STD) $(WARN) -g -O0 -DPRJ_DEBUG $(MACHINE_CFLAGS) $(OMPFLAGS)
else
CFLAGS := $(STD) $(WARN) -O3 $(MACHINE_CFLAGS) $(OMPFLAGS)
endif

TARGET ?= prj
SRC_DIR := src
TEST_DIR := tests

SRCS := \
	$(SRC_DIR)/main.c \
	$(SRC_DIR)/prj_amr.c \
	$(SRC_DIR)/prj_boundary.c \
	$(SRC_DIR)/prj_eos.c \
	$(SRC_DIR)/prj_flux.c \
	$(SRC_DIR)/prj_gravity.c \
	$(SRC_DIR)/prj_io.c \
	$(SRC_DIR)/prj_mesh.c \
	$(SRC_DIR)/prj_mhd.c \
	$(SRC_DIR)/prj_mpi.c \
	$(SRC_DIR)/prj_perturbation.c \
	$(SRC_DIR)/prj_radiation.c \
	$(SRC_DIR)/prj_rad3_opac.c \
	$(SRC_DIR)/prj_rad_inel.c \
	$(SRC_DIR)/prj_reconstruct.c \
	$(SRC_DIR)/prj_riemann.c \
	$(SRC_DIR)/prj_src.c \
	$(SRC_DIR)/prj_timeint.c \
	$(SRC_DIR)/prj_timer.c \
	$(SRC_DIR)/prj_utils.c \
	problems/prj_problem_general.c \
	problems/prj_problem_cc.c \
	problems/prj_problem_ccsn.c \
	problems/prj_problem_sedov.c \
	problems/prj_problem_sedov_offcenter.c \
	problems/prj_problem_shock1d.c

OBJS := $(SRCS:.c=.o)
CORE_SRCS := $(filter-out $(SRC_DIR)/main.c,$(SRCS))
CORE_OBJS := $(CORE_SRCS:.c=.o)
TEST_SRCS := $(wildcard $(TEST_DIR)/*.c)
TEST_BINS := $(TEST_SRCS:.c=)

.PHONY: all clean test

all: $(TARGET)

$(TARGET): $(OBJS)
	$(CC) $(CFLAGS) $(OBJS) $(LDFLAGS) $(LDLIBS) -o $@

$(SRC_DIR)/%.o: $(SRC_DIR)/%.c $(SRC_DIR)/prj.h $(SRC_DIR)/prj_defs.h $(SRC_DIR)/prj_types.h
	$(CC) $(CPPFLAGS) $(CFLAGS) -c $< -o $@

$(TEST_DIR)/%: $(TEST_DIR)/%.c $(CORE_OBJS)
	$(CC) $(CPPFLAGS) $(CFLAGS) $< $(CORE_OBJS) $(LDFLAGS) $(LDLIBS) -o $@

test: $(TEST_BINS)
	@set -e; for test_bin in $(TEST_BINS); do ./$$test_bin; done

clean:
	rm -f $(TARGET) $(OBJS) $(CORE_OBJS) $(TEST_BINS)
