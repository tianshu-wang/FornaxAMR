...
static double prj_cc_init_amr_Lrho_min(const prj_cc_init_amr_ctx *ctx, double rmin, double rmax)
{
    double Lmin;
    double Lend;
    int i;
    ...
    Lmin = prj_cc_init_amr_Lrho_interp(ctx, rmin);
    Lend = prj_cc_init_amr_Lrho_interp(ctx, rmax);
    if (Lend < Lmin) {
        Lmin = Lend;
    }
