...
int main(void)
{
    ...
    if (!prj_almost_equal(block.flux[X1DIR][VIDX(PRJ_CONS_MOM2, 1, 1, 1)], 0.0)) {
        prj_block_free_data(&block);
        return 3;
    }
    ...
    if (!prj_almost_equal(block.flux[X2DIR][VIDX(PRJ_CONS_MOM2, 1, 1, 1)], 18.0 + 2.0)) {
        prj_block_free_data(&block);
        return 5;
    }
    if (!prj_almost_equal(block.flux[X2DIR][VIDX(PRJ_CONS_MOM1, 1, 1, 1)], 0.0)) {
        prj_block_free_data(&block);
        return 6;
    }
    ...
    if (!prj_almost_equal(block.flux[X3DIR][VIDX(PRJ_CONS_MOM2, 1, 1, 1)], 0.0)) {
        prj_block_free_data(&block);
        return 8;
    }
    ...
    if (!prj_almost_equal(total[PRJ_CONS_MOM1], 0.0) ||
        !prj_almost_equal(total[PRJ_CONS_MOM2], 0.0) ||
        !prj_almost_equal(total[PRJ_CONS_MOM3], 0.0) ||
        !prj_almost_equal(total[PRJ_CONS_YE], 0.0)) {
        prj_block_free_data(&block);
        return 10;
    }
