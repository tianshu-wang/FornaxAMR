...
static int prj_neighbor_count(const prj_block *b)
{
    int n;
    int count = 0;
...
static int prj_block_contains_x(const prj_block *b, double x)
{
    return b->xmin[0] <= x && x <= b->xmax[0];
}

static int prj_has_neighbor_level(const prj_mesh *mesh, const prj_block *b, int level)
{
    int n;
    ...
    for (n = 0; n < 56; ++n) {
        ...
        if (id >= 0 && mesh->blocks[id].id >= 0 && mesh->blocks[id].active == 1 &&
...
static void prj_fill_discontinuity(prj_mesh *mesh)
{
    int bidx;
...
static void prj_fill_uniform(prj_mesh *mesh, double rho)
{
    int bidx;
...
int main(void)
{
    ...
    for (oct = 0; oct < 8; ++oct) {
        ...
        if (prj_neighbor_count(children[oct]) < 7 || prj_neighbor_count(children[oct]) > 14) {
            prj_mesh_destroy(&mesh);
            return 6;
        }
        active_children += 1;
    ...
    for (oct = 0; oct < adapt_mesh.nblocks; ++oct) {
        ...
        if (b->level == 2 && prj_has_neighbor_level(&adapt_mesh, b, 1)) {
            coarse_fine_found = 1;
        }
    }
    if (coarse_fine_found == 0) {
        prj_mesh_destroy(&adapt_mesh);
        return 18;
    }

    prj_fill_discontinuity(&adapt_mesh);
    prj_amr_adapt(&adapt_mesh, &eos);

    {
        int refined_blocks = 0;
        int refined_near_discontinuity = 0;

        for (oct = 0; oct < adapt_mesh.nblocks; ++oct) {
            if (adapt_mesh.blocks[oct].id >= 0 && adapt_mesh.blocks[oct].active == 1) {
                refined_blocks += adapt_mesh.blocks[oct].level > 0;
                if (adapt_mesh.blocks[oct].level > 0 &&
                    prj_block_contains_x(&adapt_mesh.blocks[oct], 0.5)) {
                    refined_near_discontinuity = 1;
                }
            }
        }
        if (refined_blocks == 0) {
        ...
        if (refined_near_discontinuity == 0) {
            prj_mesh_destroy(&adapt_mesh);
            return 19;
        }
    }
